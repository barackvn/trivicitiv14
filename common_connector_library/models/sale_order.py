...
class SaleOrder(models.Model):
    ...
    def process_orders_and_invoices_ept(self):
        ...
        for order in self:
            ...
            if not order.mapped('order_line').filtered(lambda l:l.product_id.invoice_policy == 'order'):
                continue
    ...
    def auto_shipped_order_ept(self, customers_location,
                               is_mrp_installed=False):
        """
        Added by Udit
        ...
        for order_line in order_lines:
            ...
            for bom_line in bom_lines:
                self.create_and_done_stock_move_ept(order_line, customers_location, bom_line)
            if not bom_lines or not is_mrp_installed:
    ...
    def create_and_done_stock_move_ept(self, order_line, customers_location, bom_line=False):
        """
        Added by Udit
