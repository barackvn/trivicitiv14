...
class XLSXEdit(object):
    def __init__(self, xlsx, cellutil):
        ...
        self._namespaces = {
            ...
            'relof': 'http://schemas.openxmlformats.org/officeDocument/2006/relationships',
            'reldrw': 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing',
            'relimg': 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image',
    ...
    def _add_drawing(self):
        ...
        try: 
            for img in os.listdir(os.path.join(self._zip_folder, 'xl/media')):
                if int(img.split('.')[0][5:]) > drawing['img_max']:
                    drawing['img_max'] = int(img.split('.')[0][5:]) 
                drawing['img_types'].append(img.split('.')[1])
            drawing['img_types'] = list(set(drawing['img_types']))
        except:
            pass
        xml_sheet = self._get_xml(self._sheet_paths[self._conf['sheet_reference']])
        node_drawing = xml_sheet.xpath('/ws:worksheet/ws:drawing', namespaces=self._namespaces)
        if len(node_drawing) :        
            sheet_drw_r_id = node_drawing[0].attrib.get('{%s}id'%self._namespaces['relof'])
            xml_sheet_rels = self._get_xml('xl/worksheets/_rels/%s.rels' %  self._sheet_paths[self._conf['sheet_reference']].split('/')[-1])
            pattern_params_rel = {'Id': sheet_drw_r_id, 'Type': self._namespaces['reldrw']}
            pattern_rel = '/rel:Relationships/rel:Relationship[@Id="%(Id)s" and @Type="%(Type)s"]' % pattern_params_rel
            drawing['name_drawing_xml'] = xml_sheet_rels.xpath(pattern_rel, namespaces=self._namespaces)[0].attrib['Target'].split('/')[-1]
            drawing_xml_path = 'xl/drawings/%s' %  drawing['name_drawing_xml'] 
            xml_drawings = self._get_xml(drawing_xml_path)
            nodes_cNvPr = xml_drawings.xpath('//xdr:nvPicPr/xdr:cNvPr', namespaces=self._namespaces)
            for node_cNvPr in nodes_cNvPr:
                if int(node_cNvPr.attrib['id']) > drawing['nvPicPr_id_max']:
                    drawing['nvPicPr_id_max'] = int(node_cNvPr.attrib['id'])
            drawing_xml_rels_path = 'xl/drawings/_rels/%s.rels' %  drawing['name_drawing_xml']            
            xml_drawings_rel = self._get_xml(drawing_xml_rels_path)
            pattern_drawings_rel = '/rel:Relationships/rel:Relationship'
            for node_Relationship in xml_drawings_rel.xpath(pattern_drawings_rel, namespaces=self._namespaces):
                if int(node_Relationship.attrib['Id'][3:]) > drawing['drw_rid_max_rel']:
                    drawing['drw_rid_max_rel'] = int(node_Relationship.attrib['Id'][3:]) 
        else:
            try: 
                drawing_xml_max = 0
                for drw in os.listdir(os.path.join(self._zip_folder, 'xl/drawings')):
                    if os.path.isfile(os.path.join(os.path.join(self._zip_folder, 'xl/drawings'), drw)):
                        if int(drw.split('.')[0][7:]) > drawing_xml_max:
                            drawing_xml_max = int(drw.split('.')[0][7:]) 
                drawing['name_drawing_xml'] = 'drawing%s.xml' % str(drawing_xml_max+1)
            except:
                drawing['name_drawing_xml'] = 'drawing%s.xml' % str(drawing_xml_max+1)
            drawing_xml_path = 'xl/drawings/%s' %  drawing['name_drawing_xml'] 
            drawing_xml_content = '''<xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"></xdr:wsDr>'''            
            xml_drawings = etree.fromstring(drawing_xml_content).getroottree()
            drawing_xml_rels_path = 'xl/drawings/_rels/%s.rels' %  drawing['name_drawing_xml']
            drawing_xml_rels_content = '''<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"></Relationships>'''
            xml_drawings_rel = etree.fromstring(drawing_xml_rels_content).getroottree()
            xml_sheet_rels_rid_max = 0
            sheet_rels_path = 'xl/worksheets/_rels/%s.rels' %  self._sheet_paths[self._conf['sheet_reference']].split('/')[-1] 
            try:
                xml_sheet_rels = self._get_xml(sheet_rels_path)
                pattern_sheet_rel = '/rel:Relationships/rel:Relationship'
                for node_Relationship in xml_sheet_rels.xpath(pattern_sheet_rel, namespaces=self._namespaces):
                    if int(node_Relationship.attrib['Id'][3:]) > xml_sheet_rels_rid_max:
                        xml_sheet_rels_rid_max = int(node_Relationship.attrib['Id'][3:])                 
                pattern_rel = '/rel:Relationships'
                node_Relationships = xml_sheet_rels.xpath('/rel:Relationships', namespaces=self._namespaces)
                node_Relationship = etree.SubElement(node_Relationships[0], '{http://schemas.openxmlformats.org/package/2006/relationships}Relationship', 
                                                     Id="rId%s"% str(xml_sheet_rels_rid_max + 1),
                                                     Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing",
                                                     Target="../drawings/%s" %  drawing['name_drawing_xml'],
                                                     )
            except:
                new_sheet_rels_content = '''<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
                                        <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" Target="../drawings/%s"/>
                                      </Relationships>
                                    ''' %  drawing['name_drawing_xml']
                xml_sheet_rels = etree.XML(new_sheet_rels_content, parser).getroottree()
            self._write_xml(sheet_rels_path, xml_sheet_rels)
            node_wsh = xml_sheet.xpath('/ws:worksheet', namespaces=self._namespaces)
            node_drawing = etree.SubElement(node_wsh[0], '{http://schemas.openxmlformats.org/spreadsheetml/2006/main}drawing')
            node_drawing.attrib['{http://schemas.openxmlformats.org/officeDocument/2006/relationships}id'] = "rId%s"% str(xml_sheet_rels_rid_max + 1)
            ws_arr = [
                    "sheetPr",
                    "dimension",
                    "sheetViews",
            ...
            for n in ws_arr:
                node_ws_prev = xml_sheet.xpath('/ws:worksheet/ws:%s'% n, namespaces=self._namespaces)        
                if len(node_ws_prev):
                    node_wsh[0].insert(node_wsh[0].index(node_ws_prev[0])+1, node_drawing)  
                    break
            self._write_xml(self._sheet_paths[self._conf['sheet_reference']], xml_sheet)
        img_types = []
        for k,v in list(self._data_drawing[self._conf['sheet_reference']].items()):
            img_stream = BytesIO(base64.b64decode(v['value']))
            img = PILImage.open(img_stream)
            if img.format.lower() in ['gif', 'jpg', 'jpeg', 'png']:         
                drawing['img_max'] += 1 
                img_name = 'image' + str(drawing['img_max']) +'.' + img.format.lower()
                rel_path = 'xl/media/' + img_name 
                img_types.append(img.format.lower())    
                os.makedirs(os.path.dirname(os.path.join(self._zip_folder, rel_path)), exist_ok=True)
                img.save(os.path.join(self._zip_folder, rel_path))
                drawing['drw_rid_max_rel'] += 1 
                node_Relationships = xml_drawings_rel.xpath('/rel:Relationships', namespaces=self._namespaces)
                node_Relationship = etree.SubElement(node_Relationships[0], '{http://schemas.openxmlformats.org/package/2006/relationships}Relationship', 
                                                     Id="rId%s"% str(drawing['drw_rid_max_rel']),
                                                     Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image",
                                                     Target="../media/" + img_name,
                                                     )
                node_wsDr = xml_drawings.xpath('/xdr:wsDr', namespaces=self._namespaces)
                drawing['nvPicPr_id_max'] += 1 
                params_oneCellAnchor = {'col': str(self.column_index_from_string(self.coordinate_from_string(k)[0])-1),
                                        'row': str(self.coordinate_from_string(k)[1]-1),
                                        'colOff': str(v['DrawingML']['colOff']),
                                        'rowOff': str(v['DrawingML']['rowOff']),
                                        'cx': str(v['DrawingML']['cx']),
                                        'cy': str(v['DrawingML']['cy']),
                                        'id': '%s' % str(drawing['nvPicPr_id_max']),
                                        'name': 'Picture %s' % str(drawing['drw_rid_max_rel']),
                                        'embed': "rId%s" % str(drawing['drw_rid_max_rel']),
                                        }
                drawing_xml_new_oneCellAnchor = '''
                <xdr:wsDr
                    xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing"
                    xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
                    <xdr:oneCellAnchor>
                        <xdr:from>
                            <xdr:col>%(col)s</xdr:col>
                            <xdr:colOff>%(colOff)s</xdr:colOff>
                            <xdr:row>%(row)s</xdr:row>
                            <xdr:rowOff>%(rowOff)s</xdr:rowOff>
                        </xdr:from>
                        <xdr:ext cx="%(cx)s" cy="%(cy)s"/>
                        <xdr:pic>
                            <xdr:nvPicPr>
                                <xdr:cNvPr id="%(id)s" name="%(name)s"/>
                                <xdr:cNvPicPr>
                                    <a:picLocks noChangeAspect="1"/>
                                </xdr:cNvPicPr>
                            </xdr:nvPicPr>
                            <xdr:blipFill>
                                <a:blip xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" r:embed="%(embed)s"/>
                                <a:stretch>
                                    <a:fillRect/>
                                </a:stretch>
                            </xdr:blipFill>
                ...
                </xdr:wsDr>''' % params_oneCellAnchor
                node_wsDr[0].append(etree.XML(drawing_xml_new_oneCellAnchor, parser).getroottree().xpath('/xdr:wsDr/xdr:oneCellAnchor', namespaces=self._namespaces)[0])
        self._write_xml(drawing_xml_rels_path, xml_drawings_rel)
        self._write_xml(drawing_xml_path, xml_drawings)
        xml_Content_Types = self._get_xml('[Content_Types].xml')
        node_Types = xml_Content_Types.xpath('/ct:Types', namespaces=self._namespaces)[0]
        img_types = list(set(img_types) - set(drawing['img_types']))
        Content_Types_types = { 
        ...
        for t in img_types:
            if t in Content_Types_types:
                node_Types.append(etree.fromstring(Content_Types_types[t]))
        PartName = False
        for node in xml_Content_Types.xpath('/ct:Types/ct:Override', namespaces=self._namespaces):
            if node.attrib.get('PartName') == "/xl/drawings/%s" % drawing['name_drawing_xml']:
                PartName = True
                break        
        if not PartName:
            node_Types.append(etree.fromstring('<Override PartName="/xl/drawings/%s" ContentType="application/vnd.openxmlformats-officedocument.drawing+xml"/>' % drawing['name_drawing_xml']))
        self._write_xml('[Content_Types].xml', xml_Content_Types)
    def _write_xml(self, rel_path, xml_content):
        os.makedirs(os.path.dirname(os.path.join(self._zip_folder, rel_path)), exist_ok=True)
        with open(os.path.join(self._zip_folder, rel_path), 'wb') as xml_out:
            xml_content.write(xml_out,
                            xml_declaration=True, 
                            encoding="UTF-8", 
                            standalone="yes")                
