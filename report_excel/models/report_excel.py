...
class ReportExcel(models.Model):
    ...
    out_file_name = fields.Char(string='Output File Name', default="'Report_%d-%m-%Y'", help='''The "Output File Name" uses Python syntax \n
The following variables can be used:
- obj: is the current record of the "Root Model"
- datetime: useful Python libraries 
See bellow Legends for supported Date and Time Formats.
    ''')
    ...
    report_lang_id = fields.Many2one(comodel_name="res.lang", string="Report Language", required=False,
                                   help="You can force the Report Language to be different from the Language of the User who prints the Report.",
...
class ReportExcelSection(models.Model):
    ...
    archive_bool = fields.Boolean(string='Print Archived Data', default=False, help="By default, only Active Records are printed in the Report. \nIf you want Archived Records to be printed as well, check the box here.")    
    ...
    def _check_start_end(self):
        ...
        for section in self:
            coord_string_start = section.section_start 
            ...
            if not match:
                ...
                '''  % (coord_string_start, section.name,) 
                raise ValidationError(msg)
            column_start, row_start = match.groups()
            row_start = int(row_start)
            if not row_start:
                msg = "There is no row 0 . Invalid Start Section coordinates: \"%s\",  Section: \"%s\" !"  % (coord_string_start, section.name,)
                raise ValidationError(msg)
            coord_string_end = section.section_end 
            ...
            if not match:
                ...
                '''  % (coord_string_end, section.name,) 
                raise ValidationError(msg)
            column_end, row_end = match.groups()
            row_end = int(row_end)
            if not row_end:
                msg = "There is no row 0 . Invalid End Section coordinates: \"%s\",  Section: \"%s\" !"  % (coord_string_end, section.name,)
                raise ValidationError(msg)
            if row_start > row_end or CheckCell.column_index_from_string(column_start) > CheckCell.column_index_from_string(column_end):
                ...
                '''  % (coord_string_start, coord_string_end, section.name,) 
                raise ValidationError(msg)
        return 
class ReportExcelFields(models.Model):
    ...
    def _compute_aggregate_ids(self):
        for field in self:
            domain = []
            if not field.field_type or field.field_type in ('integer', 'float', 'monetary'):
                pass
            ...
            if len(res_having_param.ids): 
                field.having_param_ids = res_having_param.mapped('id')
            else:
                field.having_param_ids = None
...
\u0023   result = cell(H7) ''',
    ...
    image_bool = fields.Boolean(string='Image', default=False, help='''Check the box if the field value is an Image.\n
    ''')
    colOff = fields.Integer(string='Column Offset', default=0, help="This value is used to specify the column offset within a cell in EMUs. EMUs (English Metric Units) are a high precision coordinate space (914400 dpi / 360000 dpc).")
    rowOff = fields.Integer(string='Row Offset', default=0, help="This value is used to specify the row offset within a cell in EMUs. EMUs (English Metric Units) are a high precision coordinate space (914400 dpi / 360000 dpc).")
    cx = fields.Integer(string='Extent Length', default=1500000, help="Specifies the length of the extents image in EMUs. This length shall dictate the size of the object as displayed (the result of any scaling to the original object). EMUs (English Metric Units) are a high precision coordinate space (914400 dpi / 360000 dpc).")
    cy = fields.Integer(string='Extent Width', default=1500000, help="Specifies the width of the extents image in EMUs. This width shall dictate the size of the object as displayed (the result of any scaling to the original object). EMUs (English Metric Units) are a high precision coordinate space (914400 dpi / 360000 dpc).")
    description = fields.Text('Description')
    ...
    def _onchange_aggregate_domain(self):
        ...
        if not self.field_type or self.field_type in ('integer', 'float', 'monetary'):
    ...
    def _check_format(self):
        ...
        for rec in self:
            ...
            return 
    @api.model
    @api.onchange('image_bool')
    def _set_image_restrict(self):
        if self.image_bool:
            self.sort_by = False
            self.formula = False
    @api.model
    @api.onchange('colOff','rowOff','cx','cy',)
    def _set_positive(self):
        self.colOff = self.colOff * -1 if self.colOff and self.colOff < 0 else self.colOff
        self.rowOff = self.rowOff * -1 if self.rowOff and self.rowOff < 0 else self.rowOff
        self.cx = self.cx * -1 if self.cx and self.cx < 0 else self.cx
        self.cy = self.cy * -1 if self.cy and self.cy < 0 else self.cy
