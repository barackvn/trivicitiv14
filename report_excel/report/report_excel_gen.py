...
from odoo.tools.safe_eval import datetime as s_datetime, dateutil as s_dateutil, json as s_json, time as s_time, pytz as s_pytz
...
class ReportExcelGen(models.TransientModel):
    ...
    def create_xls(self, datas=None, cellutil=None):
        ...
        for record in records:
            try:
                f_name = safe_eval(res_conf['out_file_name'],{'obj': record, 'time': s_time, 'datetime': s_datetime.datetime})
                out_file_name = safe_eval("pytz.utc.localize(datetime.now(), is_dst=None).astimezone(tz).replace(tzinfo=None).strftime(f_name)",{'obj': record, 'time': s_time, 'datetime': s_datetime.datetime, 'f_name': f_name, 'pytz':s_pytz, 'tz':tz})
                break
            except:
                _logger.info("Error or Old 'Output File Name' Expression Definition Syntax: %s  !", res_conf['out_file_name'])
                out_file_name = pytz.utc.localize(datetime.now(), is_dst=None).astimezone(tz).replace(tzinfo=None).strftime(res_conf['out_file_name'])
                break            
        else:
            out_file_name = pytz.utc.localize(datetime.now(), is_dst=None).astimezone(tz).replace(tzinfo=None).strftime(res_conf['out_file_name'])
        ...
        if template_xlsx:
            ...
            if 'macroEnabled' in template.mimetype:  
                file_ext =  '.xlsm'
        else:
        ...
        out_file_name = out_file_name + file_ext
        ...
        conf_data['conf']['report_lang_code'] =  res_conf.report_lang_id.code if res_conf.report_lang_id.code else self.env.context.get('lang')
        ...
        for i in res_conf.report_excel_section_ids:
            conf_data['conf']['section'][i['id']] = self._get_section_conf(i['id'], conf_data['conf']['report_params'], xlsx_template, None, Drawing_ML)
            ...
            for col in res_data:
                for cell in col:
                    if cell[0][0] is not None and cell[1]['value'] is not None:
                        xlsx_template.write(conf_data_out['conf']['sheet_reference'], xlsx_template.cell_from_coordinate(cell[0][1], cell[0][3]) , cell[1]['value'], cell_ext=cell, DrawingML=Drawing_ML.Pictures[cell[0][0]])    
        ...
        if datas['send_by_email']:
            ...
            return file_obj
    def _get_section_conf(self, section_id, params=None, xlsx_template=None, chain_group=None, Drawing_ML=None):
        conf_section = {}
        ...
        conf_section  = {
            ...
            'children_ids': {d['id']:d for d in ([self._get_section_conf(p, params, xlsx_template, chain_group, Drawing_ML) for p in section['children_ids']] if len(section['children_ids']) else section['children_ids'])},
            'section_start': section['section_start'],
            'section_end': section['section_end'],
            'level': section['level'],
            'archive_bool': section['archive_bool'],
            'domain': self._get_domain(section['domain'], params),
            'report_excel_fields_ids': {d[0][0]:d for d in ([self._get_fields_conf(p, params, xlsx_template, Drawing_ML) for p in section['report_excel_fields_ids']] if len(section['report_excel_fields_ids']) else section['report_excel_fields_ids'])},
    ...
    def _get_domain(self,domain, params):
        ...
        return domain_arr
    def _get_fields_conf(self, field_id, params=None, xlsx_template=None, Drawing_ML=None):
        conf_field = []
        ...
        for n in fields:
            ...
            conf_field = [self._get_conf_coordinate(xlsx_template, n.cell), 
                          ...
                          'cell_start': {},
                          'image_bool': n.image_bool,
                          'colOff': n.colOff,
                          'rowOff': n.rowOff,
                          'cx': n.cx,
                          'cy': n.cy,
                          }]
            Drawing_ML(n.cell, conf_field)
        return conf_field
    ...
    def _get_section_data(self, section_id, conf=None, active_ids=(), xlsx_template=None, order=[], parent_data_line_id=None, res={}, chain_ids=(), Models_Env={}, Fields_Env={}):
        ...
        domain_ext = expression.AND([conf_active['domain'], [('id', 'in', active_ids)]]) if len(active_ids) else conf_active['domain']             
        ...
        res_root_lines = []
        for record in self.env[conf_active['root_model_name']].with_context(active_test=False).search(domain_ext) if conf_active['archive_bool'] else self.env[conf_active['root_model_name']].search(domain_ext):
            rec_dict_tmp = {}
        ...
        for data_line in res_root_lines:
            ...
            data.append([[parent_data_line_id], (chain_ids_tpl,), [data_line['id']], self._get_field_data(data_line, conf_active, xlsx_template, Models_Env, Fields_Env, conf['report_lang_code'])])
            for child_section_id in conf_active['children_ids']:
                child_conf = self._get_active_conf(child_section_id, conf, order)
                root_field_name = self._get_model(Models_Env, 'ir.model.fields', conf['report_lang_code'])[child_conf['root_model_related_field_id'][0]].name
                root_active_ids = []
        ...
        for d in data:    
            res['data'].append(d)         
        return res
    def _get_field_data(self, res, conf, xlsx_template, Models_Env, Fields_Env, lang):
        val = {}
        for k, v in list(conf['report_excel_fields_ids'].items()):
            field_split = tuple(v[2].split("."))
            val[k] = [self._get_conf_coordinate(xlsx_template, k), self._get_data(res['id'], conf['root_model_name'], field_split, Models_Env, Fields_Env, lang)]
        return val
    def _get_data(self, id, model, field_split, Models_Env, Fields_Env, lang):
        res_test = self._get_model(Models_Env, model, lang).get(id)
        if res_test is None:
            return ((None,'',),)
        else:
            res = {}
        ...
        try:
            field = self._get_field(Fields_Env, model, field_split[0], lang)
        except:
        ...
        if len(field_split) > 1:
            ...
            if field['type'] == 'many2one':
                if res[field_split[0]]:  
                    res_value = self._get_data(res[field_split[0]][0], field['relation'], field_split_ext, Models_Env, Fields_Env, lang)
                else:
                    res_value = ((None,'',),)
            else:
                if len(res[field_split[0]]):
                    ...
                    for p in res[field_split[0]]:
                        res_value = res_value + self._get_data(p, field['relation'], field_split_ext, Models_Env, Fields_Env, lang)
                else:
                    res_value = ((None,'',),)
            return res_value
        else:
            if field['type'] == 'many2one':
                ...
                res_value = self._get_x2many(model, field['relation'], field_split[0], res['id'], Models_Env, lang)
                ...
                if res[field_split[0]]:
                    res_value = ((None, pytz.utc.localize(res[field_split[0]], is_dst=None).astimezone(pytz.timezone(self._context.get('tz') or self.env.user.tz or 'UTC')).replace(tzinfo=None),),) if field['type'] == 'datetime' else ((None, res[field_split[0]],),)
                else:
                    res_value = ((None,'',),)
    ...
    def _post_processing_data(self, section_id, conf=None, data=None, parent_chain_ids=None, having_remove_ids=None):
                    ...
                    for cell_val in dl[3][k][1]:
                        ...
                        while f.find('cell(') > -1:
                            ...
                            if cell != None:
                                ...
                                raise UserError(msg)
                        try:
                            f = f.replace('.now()', '.now().replace(tzinfo=pytz.utc).astimezone(tz).replace(tzinfo=None)')
                            globaldict = {
                                'datetime': s_datetime.datetime,
                                'date': s_datetime.date,
                                'dateutil': s_dateutil,
                                'uid': self.env.uid,
                                'user': self.env.user,
                                'json': s_json,
                            ...
                            n_val =  localdict['result'] if 'result' in localdict and type(localdict['result']) in (int, float,) else 'result' in localdict and localdict['result'] or ''   
        ...
        if len(sort_by_group):
            ...
            for s in sort_by_group_sorted:
                ...
                reverse = False if s[2] == 'asc' else True 
                for group in groups:
                    try:
                        if len(group) > 1:
            ...
            if section_id in data['children_ids']:
                ...
                if len(data['children_ids']):
                    for k,v in list(data['children_ids'].items()):
                        ...
                        if item is not None:
                            return item                         
    def _get_x2many(self, model_name, comodel_name, name_field, param_id, Models_Env, lang):
        record = self._get_model(Models_Env, model_name, lang)[param_id]
        attrs_str = ''
        attrs_ids = None
        if len(record):
            ...
            for r in records_ids:
                attrs.append(self._get_model(Models_Env, comodel_name, lang)[r])
            i = 1
            for p in attrs:
    ...
    def _get_model(self, Models_Env, model_name, lang=None):
        if model_name in Models_Env.models:
            return Models_Env.models.get(model_name)
        else:
            model_ids = self.env[model_name].with_context(active_test=False,lang=lang).search([]) if self.env[model_name]._fields.get('active') else self.env[model_name].with_context(lang=lang).search([]) 
            model_ind = {}
            for r in model_ids:
                model_ind[r.id] = r
            Models_Env(model_name, model_ind)
            return Models_Env.models.get(model_name)
    def _get_field(self, Fields_Env, model_name, field_name, lang=None):
        if model_name in Fields_Env.models and field_name in Fields_Env.models[model_name]:
            ...
            field = self.env[model_name].with_context(lang=lang).fields_get([field_name])[field_name]
...
class DrawingML(object):
    def __init__(self):
        self.Pictures = {}
        self.Locked_Canvases = {}
        self.Diagrams = {}
        self.Charts = {}
    def __call__(self,  cell, conf_field, object_name='Pictures'):
        if object_name =='Pictures':
            if cell not in self.Pictures:
                self.Pictures[cell] = {
                    'SpreadsheetML': 'Pictures',
                    'field_type': conf_field[4]['field_type'], 
                    'image_bool': conf_field[4]['image_bool'], 
                    'colOff': conf_field[4]['colOff'], 
                    'rowOff': conf_field[4]['rowOff'], 
                    'cx': conf_field[4]['cx'], 
                    'cy': conf_field[4]['cy'],
                    }
        if object_name =='Locked_Canvases':
            if cell not in self.Locked_Canvases:
                self.Locked_Canvases[cell] = {
                    'SpreadsheetML': 'Locked_Canvases',
                    'field_type': conf_field[4]['field_type'], 
                    }
        if object_name =='Diagrams':
            if cell not in self.Diagrams:
                self.Diagrams[cell] = {
                    'SpreadsheetML': 'Diagrams',
                    'field_type': conf_field[4]['field_type'], 
                    }
        if object_name =='Charts':
            if cell not in self.Charts:
                self.Charts[cell] = {
                    'SpreadsheetML': 'Charts',
                    'field_type': conf_field[4]['field_type'], 
                    }
