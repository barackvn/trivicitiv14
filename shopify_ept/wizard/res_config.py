...
class shopify_instance_config(models.TransientModel):
    ...
    def test_shopify_connection(self):
        ...
        pricelist_id = pricelist_obj.search([('name', '=', price_list_name), ('currency_id', '=', self.env.user.currency_id.id)], limit=1)
        if not pricelist_id:
        ...
        self.env['shopify.instance.ept'].create(vals)             
        return {
...
#    _name = 'shopify.config.settings'
    ...
    shopify_company_id = fields.Many2one('res.company', string='Company', default=_get_default_company, help="Orders and Invoices will be generated of this company.")
    ...
    auto_closed_order = fields.Boolean("Auto Closed Order", Default=False, help="If checked, It will closed order in Shopify while update order status")
    ...
    shopify_order_import_next_execution = fields.Datetime('Next Execution', help='Next execution time')
    shopify_order_import_user_id = fields.Many2one('res.users', string="User", help='User', default=lambda self: self.env.user)
    ...
    shopify_order_update_next_execution = fields.Datetime('Next Execution', help='Next execution time')
    shopify_order_update_user_id = fields.Many2one('res.users', string="User", help='User', default=lambda self: self.env.user)
    ...
    shopify_property_account_payable_id = fields.Many2one('account.account', string="Shopify Account Payable", help='This account will be used instead of the default one as the payable account for the current partner')
    shopify_property_account_receivable_id = fields.Many2one('account.account', string="Shopify Account Receivable", help='This account will be used instead of the default one as the receivable account for the current partner')
    shopify_apply_tax_in_order = fields.Selection([("odoo_tax", "Odoo Tax"), ("create_shopify_tax",
                                                                              "Create New Tax")],
                                                  default="create_shopify_tax", copy=False, help="If select odoo "
                                                                                       "tax, In order "
                                                                                       "line set the tax "
                                                                                       "base on Odoo "
                                                                                       "tax configuration "
    ...
    def onchange_shopify_instance_id(self):
        ...
        if instance:
            ...
            try:
                inventory_cron_exist = self.env.ref('shopify_ept.ir_cron_auto_export_inventory_instance_%d' % (instance.id))
            except:
            ...
            try:
                shopify_order_import_cron_exist = self.env.ref('shopify_ept.ir_cron_import_shopify_orders_instance_%d' % (instance.id))
            except:
            ...
            try:
                shopify_order_update_cron_exist = self.env.ref('shopify_ept.ir_cron_auto_update_order_status_instance_%d' % (instance.id))
            except:
    ...
    def execute(self):
        ...
        if not self.env.user.has_group('shopify_ept.group_shopify_ept') and self.env.user.has_group('shopify_ept.group_shopify_manager_ept'):
            return res 
        if instance:
            ...
            values['is_use_default_sequence'] = self.is_use_default_sequence and self.is_use_default_sequence
