...
class shopify_product_template_ept(models.Model):
    ...
    inventory_management = fields.Selection(
        ...
        default='shopify')
    check_product_stock = fields.Boolean("Sale out of stock products ?", default=False, help='If true than customers are allowed to place an order for the product variant when it is out of stock.')
        ...
        if not odoo_product and not shopify_product:
            ...
            for product_sku_barcode in product_sku_barcodes:
                ...
                if not sku and not barcode:
                    ...
                    template_title, template_id)
                    if product_count == 1:
                        message = "SKU or Barcode is not set in Product: %s and ID: %s." % (template_title, template_id)
                    importable = False
                    return importable, message
...
#             if shopify_product and odoo_products and len(
#                     product_sku_barcodes) != len(odoo_products):
#                 message = "Same SKU(s) or Barcode(s) Already exists or product in odoo for Product: %s and ID: %s." % (
#                 template_title, template_id)
#                 importable = False
#                 return importable, message
            total_shopify_sku = len(set(shopify_skus))
            ...
            if image.get('src'):
                ...
                if shopify_gallery_image:
                    shopify_gallery_image.write({'position': image.get('position'), 'height': image.get('height'),
                                                 'width': image.get('width'), 'image_id': img, 'url': image.get('src'),
                                                 'shopify_variant_ids': shopify_product_ids and shopify_product_ids})
                    
                else:
                    # this line for change shopify image if change in shopify store
                    # this is only for extra imgae changes
                    shopify_gallery_image = shopify_product_img.search(
                                                    [('shopify_product_tmpl_id', '=', shopify_template.id),
                    ...
                    if shopify_gallery_image:
                        ...
                        shopify_product_img.create(
                            {'shopify_product_tmpl_id': shopify_template.id, 'shopify_instance_id': instance.id,
                             'image_id': img, 'url': image.get('src'), 'shopify_image_id': image.get('id'), 'position': image.get('position'),
                             'height': image.get('height'), 'width': image.get('width'),
                             'shopify_variant_ids': shopify_product_ids and shopify_product_ids})
                if image.get('position') == 1:
                    shopify_template.product_tmpl_id.write({'image': img})
                total_shopify_image_id.append(shopify_image_id)
        # this for when delete extra image in shopify store
        existing_shopify_gallery_image = [] 
        for shopify_image_id in total_shopify_image_id:
            shopify_gallery_image = shopify_product_img.search(
                    [('shopify_product_tmpl_id', '=', shopify_template.id),
                     ('shopify_image_id', '=', shopify_image_id)], limit=1)
            if shopify_gallery_image:
                existing_shopify_gallery_image.append(shopify_gallery_image)
        delete_shopify_gallery_record = []
        for shopify_gallery_image in shopify_template.shopify_gallery_image_ids:
            if shopify_gallery_image not in existing_shopify_gallery_image:
                delete_shopify_gallery_record.append(shopify_gallery_image)
        if delete_shopify_gallery_record:
            for delete_record in delete_shopify_gallery_record:
                    ...
                    if instance.auto_import_product:
                        if not onetime_call:
                            if len(response_template.get('variants')) > 1:
                                ...
                                if sku or barcode:
                                    new_templ_obj = product_template_obj.create({'name': template_title,
                                                                                 'default_code': sku or '',
                                                                                 'barcode': barcode or '',
                                                                                 'type': 'product',
                                                                                 'categ_id':product_category.id
                ...
                if not shopify_variant:
                    ...
                    if not shopify_template:
                        if inventory_policy == 'continue':
                            ...
                            vals.update({'check_product_stock': False})
...
#                              'fulfillment_service': fulfillment_service,
                    ...
                    if not updated_template:
                        vals = {}
                        vals.update({'name': template_title,
#                                      'fulfillment_service': fulfillment_service,
...
#             location_ids.append(instance.warehouse_id.lot_stock_id.id)
            ...
            for location_id in location_ids:
                ...
                for template in shopify_products:
                    if template.inventory_management == 'Dont track Inventory':
                        continue
                    try:
                        ...
                        if variant.variant_id and variant.product_id.type == 'product':
                            if not variant.inventory_item_id:
                                ...
                                if not log:
                                    ...
                                    log.write({'message':message})
                                continue
                            quantity = self.get_stock(variant, location_id.warehouse_id.id, instance.stock_field.name)
                            try:
    ...
    def update_products_in_shopify(self, instance, templates):
        ...
        for template in templates:
            ...
            for variant in template.shopify_product_ids:
                shopify_image_id = False
                for gallery_image in variant.shopify_template_id.shopify_gallery_image_ids:
                    if variant.id in gallery_image.shopify_variant_ids.ids:
                        shopify_image_id = gallery_image.shopify_image_id
                image_info = {}
                if variant.product_id.image:
                    key = hashlib.md5(variant.product_id.image).hexdigest()
            ...
            # extra image
            remaing_gallery_images = []
            for gallery_image in template.shopify_gallery_image_ids:
                if not gallery_image.shopify_variant_ids:
                    remaing_gallery_images.append(gallery_image)
   
            if not len(remaing_gallery_images) == 0:
                for remaing_galler_image in remaing_gallery_images:
                    if remaing_galler_image.image_id:
                        key = hashlib.md5(remaing_galler_image.image_id).hexdigest()
                        if not key in images_with_position:
                            images_with_position.update({key: image_position})
                            image_position = image_position + 1
                for image in remaing_gallery_images:
                    image_info = {}
                    if image.image_id:
                        key = hashlib.md5(image.image_id).hexdigest()
                        if key not in exist_images:
                            image_info.update({'attachment': image.image_id.decode('utf-8'),
                                               'position': images_with_position.get(key),
                                               'id':image.shopify_image_id})
                            exist_images.append(key)
                            images.append(image_info)
                        image_with_position.append({'position': images_with_position.get(key)})
                ...
                for variant in template.shopify_product_ids:
                    if variant.variant_id:
                        if variant.shopify_image_id:
                            ...
                            variants.append(info)
                new_product.id = template.shopify_tmpl_id
                # for extra variant and set shopify_image _id in image
                for gallery_image in template.shopify_gallery_image_ids:
                    if gallery_image.shopify_variant_ids:
                        gallaery_img_varinat = gallery_image.shopify_variant_ids[0].variant_id
                        for shopify_variant in variants:
                            if shopify_variant.get('id') == gallaery_img_varinat:
                                gallery_image.shopify_image_id = shopify_variant.get('image_id')
                                break
                new_product.variants = variants
                try:
                    new_product.save()
...
#            if not image.image_id and not instance.is_image_url:
            if not image.image_id and not image.url:
                continue
            shopify_image = False
...
class shopify_product_product_ept(models.Model):
    ...
    # Added_by_jay_makeana 25/01/209
