...
class sale_order(models.Model):
        ...
        if is_company:
            ...
            if not partner:
                ...
                partner = partner_obj.search([('name', '=ilike', name), ('city', '=', city),
                                              ('street', '=', address1), ('zip', '=', zip),
                                              ('state_id', '=', state.id)] + domain, limit=1,
                                             order="id desc")
            if partner:
                partner.write({'name': name, 'opt_out': not opt_out and True or False,
                ...
                instance.shopify_property_account_receivable_id and partner.write({
                                                                                      'property_account_receivable_id': instance.shopify_property_account_receivable_id.id})
                instance.shopify_property_account_payable_id and partner.write({
                ...
                partner = partner_obj.create({'shopify_customer_id': customer_id, 'name': name,
                                              'opt_out': not opt_out and True or False,
                                              'state_id': state and state.id or False, 'city': city,
            ...
            address = self.env['res.partner'].search(
                [('name', '=', name), ('state_id', '=', state.id), ('city', '=', city),
                 ...
                 ('country_id', '=', country.id)] + domain, limit=1)
            if not address:
    ...
    @api.model
    def create_sale_order_line(self, line, product, quantity, name, order, price, order_response,
                               is_shipping=False):
        ...
        if order.shopify_instance_id.apply_tax_in_order == 'create_shopify_tax':
            taxes_included = order_response.get('taxes_included') or False
            tax_ids = []
            if line and line.get('taxable'):
                tax_ids = self.shopify_get_tax_id_ept(order.shopify_instance_id, line.get('tax_lines'),
                                                      taxes_included)
            elif not line:
                if order.shopify_instance_id.add_discount_tax:
                    tax_ids = self.shopify_get_tax_id_ept(order.shopify_instance_id,
        ...
        order_line = sale_order_line_obj.create(line_vals)
    ...
    def create_order(self, result, invoice_address, instance, partner, shipping_address,
        ...
        if not workflow and not no_payment_gateway:
            ...
            return False
        created_at = dateutil.parser.parse(result.get('created_at'))
        date_order = (created_at.astimezone(timezone('UTC'))).strftime('%Y-%m-%d %H:%M:%S')
        ordervals = {
    ...
    def auto_import_sale_order_ept(self, ctx={}):
        ...
        if shopify_instance_id:
            ...
            # Add by Haresh Mori
            # it's set to date from instance
            from_date = instance.last_date_order_import
            to_date = datetime.strptime(
                datetime.now(timezone(self.env.user.tz)).strftime("%Y-%m-%d %H:%M-%S"),
                "%Y-%m-%d %H:%M-%S")
    ...
    def import_shopify_orders(self, from_date, to_date, instance=False):
        ...
        for instance in instances:
            ...
            # Add by Haresh Mori
            # Base on configuration it's import order,order status like 'shipped','unshipped'...
            for status in instance.import_shopify_order_status_ids:
                ...
                if shopify_fulfillment_status == 'any' or shopify_fulfillment_status == 'shipped':
                    try:
                        ...
                        message = "While importing order API request is not reached to Shopify store"
                        log = transaction_log_obj.search(
                            [('shopify_instance_id', '=', instance.id), ('message', '=', message)])
                        if not log:
                            transaction_log_obj.create(
                    ...
                    try:
                        order_ids = shopify.Order().find(
                            fulfillment_statu=shopify_fulfillment_status, created_at_min=from_date,
                            created_at_max=to_date, limit=250)
                    except Exception as e:
                        message = "While importing order API request is not reached to Shopify store"
                        log = transaction_log_obj.search(
                ...
                for order_id in order_ids:
                    ...
                    log = False
                    # Add By Haresh Mori
                    # it's set shopify location in sale order
                    ...
                    for line in lines:
                        ...
                        self.create_sale_order_line(line, product, line.get('quantity'),
                                                    product.name, order, line.get('price'), result)
                    if flag:
                        order.unlink()
                        continue
                    if not risk_result:
            ...
            for sale_order in sales_orders:
                ...
                for picking in picking_ids:
                    ...
                    try:
                        ...
                        message = "While update order status  API request is not reached to Shopify store"
                        log = transaction_log_obj.search(
                            [('shopify_instance_id', '=', instance.id), ('message', '=', message)])
                        if not log:
                            transaction_log_obj.create(
    ...
    def update_carrier(self):
        ...
        for instance in instances:
            ...
            try:
                ...
                message = "While update carrier API request is not reached to Shopify store instance is :%s" % (
                    instance.name)
                log = transaction_log_obj.search(
                    [('shopify_instance_id', '=', instance.id), ('message', '=', message)])
                if not log:
    ...
    def delivery_set(self):
        if self.shopify_order_id:
            raise UserError(
                _('You are not allow to chagne manually shipping charge in Shopify order.'))
        else:
