...
class ShopifyPaymentReportEpt(models.Model):
    ...
    reconciliation_log_ids = fields.One2many('shopify.payout.logline.ept', 'payout_id',
                                             string="Reconciliation log lines")
    ...
    def generate_remaining_bank_statement(self):
        ...
        for transaction in remaining_transaction_ids:
            ...
            shopify_account_config_id = self.instance_id.transaction_line_ids.filtered(
                lambda l: l.transaction_type == transaction.transaction_type)
    ...
    def generate_bank_statement(self):
        ...
        for transaction in self.payout_transaction_ids:
            ...
            if payment_reference:
                reference = payment_reference.name
                payment_aml_rec = payment_reference.mapped('move_line_ids').filtered(
        ...
        for statement_line in bank_statement.line_ids.filtered(lambda x: x.journal_entry_ids.ids == []):
            try:
                ...
                if ref:
                    ...
                    if payment_id:
                        payment_aml_rec = payment_id.mapped('move_line_ids').filtered(
                            lambda line: line.account_internal_type == "liquidity")
                else:
                    ...
                    for order in statement_line.shopify_order_ids:
                        if statement_line.amount < 0.0:
                            invoices += order.invoice_ids.filtered(
                                lambda record: record.type == 'out_refund' and record.state == 'posted')
                        else:
                            invoices += order.invoice_ids.filtered(
                                lambda record: record.type == 'out_invoice' and record.state == 'posted')
                    for invoice in invoices:
                        if invoice.state == 'posted':
                            payment_ids = account_payment_obj.search([('invoice_ids', 'in', invoice.ids)])
                            if payment_ids:
                                payment_aml_rec = payment_ids.mapped('move_line_ids').filtered(
                                    lambda line: line.user_type_id.type == "liquidity")
                            else:
                                move_lines = invoices.mapped('line_ids').filtered(
    ...
    def prepare_transaction_vals(self, data, payout_id, instance):
        ...
        :param payout_id: Browsable record of payout_id.
        ...
        try:
            ...
            if not log_line_id:
                log_line_id = payout_logline_obj.create({'message': message_body,
                                                         'instance_id': self.instance_id.id,
                                                         'payout_id': self.id})
            self.env['shopify.payout.logline.ept'].with_context({'is_closed': True}).create_payout_schedule_activity(
                log_line_id)
            self.is_skip_from_cron = True
            return False
        self.state = 'closed'
    ...
    def unlink(self):
        ...
        for report in self:
            if report.state != 'draft':
                raise UserError(_('You cannot delete payout report.'))
    ...
    def create(self, vals):
        ...
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code(
                'shopify.payout.report.ept') or _('New')
    ...
    def auto_import_payout_report(self, ctx={}):
        ...
        if not isinstance(ctx, dict) or 'shopify_instance_id' not in ctx:
            return True
    ...
    def auto_generate_bank_statement(self, ctx={}):
        ...
        if shopify_instance_id:
            ...
            partially_generated_report = payout_report_obj.search(
                [('state', '=', 'partially_generated'), ('instance_id', '=', instance.id),
                 ('is_skip_from_cron', '!=', True)])
