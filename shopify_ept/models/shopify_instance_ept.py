...
class shopify_instance_ept(models.Model):
    ...
    apply_tax_in_order = fields.Selection([("odoo_tax", "Odoo Tax"), ("create_shopify_tax",
                                                                      "Create New Tax")],
                                          default="create_shopify_tax", copy=False, help="If select " \
                                                                               "odoo "
                                                                               "tax, In order "
                                                                               "line set the tax "
                                                                               "base on Odoo "
                                                                               ...
                                                                               "and fiscal "
                                                                               "position. if "
                                                                               "select create new tax, It will not manage the tax base on the configuration of Odoo but it will create new tax when order import from Shopify to Odoo.")
    ...
    is_use_default_sequence = fields.Boolean("Use Odoo Default Sequence?", help="If checked,Then use default sequence of odoo for sale order create")
    ...
    import_shopify_order_status_ids = fields.Many2many('import.shopify.order.status', 'shopify_instance_order_status_rel', 'instance_id', 'status_id', "Import Order Status", help="Selected status orders will be imported from Shopify")
    update_category_in_odoo_product = fields.Boolean(string="Update Category In Odoo Product ?")

    shopify_property_account_payable_id = fields.Many2one('account.account', string="Account Payable", help='This account will be used instead of the default one as the payable account for the current partner')
    shopify_property_account_receivable_id = fields.Many2one('account.account', string="Account Receivable", help='This account will be used instead of the default one as the receivable account for the current partner')
    ...
    exported_product_ids = fields.One2many('shopify.product.template.ept', 'shopify_instance_id', domain=[('exported_in_shopify', '=', True)], string="Exported Products")
    exported_product_count = fields.Integer(compute='_count_all', string="Exported Products")
    
    ready_to_expor_product_ids = fields.One2many('shopify.product.template.ept', 'shopify_instance_id', domain=[('exported_in_shopify', '=', False)], string="Ready To Export")
    ready_to_expor_product_count = fields.Integer(compute='_count_all', string="Ready To Export")
    
    published_product_ids = fields.One2many('shopify.product.template.ept', 'shopify_instance_id', domain=[('website_published', '=', True)], string="Published")
    published_product_count = fields.Integer(compute='_count_all', string="Published")
    
    unpublished_product_ids = fields.One2many('shopify.product.template.ept', 'shopify_instance_id', domain=[('website_published', '=', False), ('exported_in_shopify', '=', True)], string="Published")
    unpublished_product_count = fields.Integer(compute='_count_all', string="UnPublished")
    
    quotation_ids = fields.One2many('sale.order', 'shopify_instance_id', domain=[('state', 'in', ['draft', 'sent'])], string="Quotations")        
    quotation_count = fields.Integer(compute='_count_all', string="Quotations")
        
    order_ids = fields.One2many('sale.order', 'shopify_instance_id', domain=[('state', 'not in', ['draft', 'sent', 'cancel'])], string="Sales Order")
    order_count = fields.Integer(compute='_count_all', string="Sales Order")
    
    risk_order_ids = fields.One2many('sale.order', 'shopify_instance_id', domain=[('state', '=', 'draft'), ('is_risky_order', '!=', False)], string="Risky Order")
    risk_order_count = fields.Integer(compute='_count_all', string="Risky Order")    
   
    confirmed_picking_ids = fields.One2many('stock.picking', 'shopify_instance_id', domain=[('state', '=', 'confirmed')], string="Confirm Pickings")
    confirmed_picking_count = fields.Integer(compute='_count_all', string="Confirm Pickings")
    assigned_picking_ids = fields.One2many('stock.picking', 'shopify_instance_id', domain=[('state', '=', 'assigned')], string="Assigned Pickings")
    assigned_picking_count = fields.Integer(compute='_count_all', string="Assigned Pickings")
    partially_available_picking_ids = fields.One2many('stock.picking', 'shopify_instance_id', domain=[('state', '=', 'partially_available')], string="Partially Available Pickings")
    partially_available_picking_count = fields.Integer(compute='_count_all', string="Partially Available Pickings")
    done_picking_ids = fields.One2many('stock.picking', 'shopify_instance_id', domain=[('state', '=', 'done')], string="Done Pickings")
    done_picking_count = fields.Integer(compute='_count_all', string="Done Pickings")
    
    open_invoice_ids = fields.One2many('account.invoice', 'shopify_instance_id', domain=[('state', '=', 'open'), ('type', '=', 'out_invoice')], string="Open Invoices")
    open_invoice_count = fields.Integer(compute='_count_all', string="Open Invoices")    

    paid_invoice_ids = fields.One2many('account.invoice', 'shopify_instance_id', domain=[('state', '=', 'paid'), ('type', '=', 'out_invoice')], string="Paid Invoices")
    paid_invoice_count = fields.Integer(compute='_count_all', string="Paid Invoices")
    
    refund_invoice_ids = fields.One2many('account.invoice', 'shopify_instance_id', domain=[('type', '=', 'out_refund')], string="Refund Invoices")
    refund_invoice_count = fields.Integer(compute='_count_all', string="Refund Invoices")
    
    product_ids = fields.One2many('shopify.product.template.ept', 'shopify_instance_id', string="Products")
    ...
    shopify_custom_collection_ids = fields.One2many('shopify.collection.ept', 'shopify_instance_id', domain=[('is_smart_collection', '=', False)], string="Shopify Collection")
    shopify_custom_collection_count = fields.Integer(compute='_count_all', string="Shopify Collection")
    shopify_smart_collection_ids = fields.One2many('shopify.collection.ept', 'shopify_instance_id', domain=[('is_smart_collection', '=', True)], string="Shopify Smart Collection")
    shopify_smart_collection_count = fields.Integer(compute='_count_all', string="Shopify Smart Collection")
    ...
    def test_shopify_connection(self):
        ...
        if len(shop) == 2:
            shop_url = shop[0] + "//" + self.api_key + ":" + self.password + "@" + shop[1] + "/admin/api/2020-01"
        else :
    ...
    def connect_in_shopify(self):
        ...
        if len(shop) == 2:
            shop_url = shop[0] + "//" + instance.api_key + ":" + instance.password + "@" + shop[1] + "/admin/api/2020-01"
        else :
            shop_url = "https://" + instance.api_key + ":" + instance.password + "@" + shop[0] + "/admin/api/2020-01"
        shopify.ShopifyResource.set_site(shop_url)        
        return True                
