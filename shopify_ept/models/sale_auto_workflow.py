...
class sale_auto_workflow_configuration(models.Model):
    ...
    payment_gateway_id=fields.Many2one("shopify.payment.gateway","Payment Gateway",required=1)
    ...
    _sql_constraints=[('_workflow_unique_constraint','unique(financial_status,shopify_instance_id,payment_gateway_id)',"Financial status must be unique in the list")]
